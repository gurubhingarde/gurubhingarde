...
class FlashingApp(tk.Tk):
    def __init__(self):
        ...
        self._ph2_btn = tk.Button(
            tab_bar, text="🚀  Phase 2 — JSW Phase 2VCU",
            font=("Segoe UI", 11, "bold"),
            bg="#163c69", fg="#93C5FD",
            relief="flat", cursor="hand2", padx=18,
        ...
        _file_row(files_card, "Firmware", self.fota_ota_var,
                  [("OTA Binary", "*.bin"), ("All", "*.*")], _on_ota_selected)
        _file_row(files_card, "Security DLL", self.fota_dll_var,
                  [("DLL", "*.dll"), ("All", "*.*")])

        self.fota_info_var = tk.StringVar(value="Click on Flash to begin")
